...
def main():
    ...
    parser.add_argument(
        "--gt_dir", type=str, default="", help="Path to save the HQ images"
    )
    parser.add_argument(
        "--sr_dir", type=str, default="", help="Path to save the SR images"
    )
    ...
    for mask_type in mask_split.keys():
        num_mask_types += 1
        im_path_list = [
            (Path(args.sr_dir) / im_name) for im_name in mask_split[mask_type]
        ]
        ...
        for im_path_sr in im_path_list:
            # Load images with different normalizations based on metric requirements
            im_sr = load_im_tensor(im_path_sr)  # [-1, 1] for LPIPS
            # im_sr_01 = load_im_tensor_01(im_path_sr)  # [0, 1] for CLIPIQA and MUSIQ
            im_sr_01 = load_im_tensor(im_path_sr)

            im_path_gt = Path(args.gt_dir) / im_path_sr.name
            # im_gt = load_im_tensor(im_path_gt)  # [-1, 1] for LPIPS
            im_gt = load_im_tensor(im_path_gt)  # [-1, 1] for LPIPS
            ...
            with torch.no_grad():
                ...
                # calculate clipiqa with [0, 1] normalized images
                current_clipiqa += clipiqa_metric(im_sr_01).sum().item()
                # current_clipiqa += clipiqa_metric(im_sr_01).item()
                # calculate musiq with [0, 1] normalized images
                current_musiq += musiq_metric(im_sr_01).sum().item()
